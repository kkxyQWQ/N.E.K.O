...
class VoiceComparePlugin(NekoPluginBase):
    ...
    def on_startup(self, **_):
        ...
        # 尝试从 PluginStore 恢复数据（同步读取）
        try:
            thr = self.store.get_sync("threshold")
            if thr is not None:
        ...
        try:
            emb = self.store.get_sync("embedding")
            if emb is not None:
                with self._lock:
    ...
    def on_shutdown(self, **_):
        ...
        try:
            self.store.set_sync("threshold", self._threshold)
            with self._lock:
                emb = self._embedding
            if emb is not None:
                self.store.set_sync("embedding", emb.tolist())
        except Exception:
            pass
    ...
    def clear(self, **_):
        ...
        try:
            self.store.delete_sync("embedding")
        except Exception:
            pass
    ...
    def set_threshold(self, value: float, **_):
        v = float(value)
        if not 0.0 <= v <= 1.0:
            return Err(SdkError("阈值须在 0~1 之间"))
        self._threshold = v
        try:
            self.store.set_sync("threshold", v)
        except Exception:
            pass
