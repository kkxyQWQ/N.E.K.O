...
| **StepFun Real-Time** | WebSocket（`wss://api.stepfun.com/v1/realtime/audio`） | リアルタイムストリーミング、低レイテンシ |
| **Qwen CosyVoice 3** | クラウドAPI（Aliyun） | カスタムスピーカー音声合成、flashモデル |
| **GPT-SoVITS** | ローカルサービス | 完全オフライン、カスタマイズ可能、v3 APIサポート |
...
リサンプラーは `soxr` `ResampleStream` を使用し、単一メッセージ内の音声チャンク間で内部状態を保持します。クロスメッセージのアーティファクトを防ぐため、新しいメッセージ境界ごとに状態がリセットされます。
...
2. システムは最初にローカルTTS（`/v1/speakers/register`）を試行（利用可能な場合）
3. Aliyun CosyVoiceクラウドAPIにフォールバックして音声クローン
4. 一意の `voice_id` が返され、キャラクター設定に保存
5. そのキャラクターの以降のTTSリクエストはすべてクローンされたボイスを使用
...
| チャンネル | モノラル |
| リサンプラー | soxr `ResampleStream`（ステートフル、高品質） |
