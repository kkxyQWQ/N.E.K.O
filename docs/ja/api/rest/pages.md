...
| パス | テンプレート | 説明 |
|------|----------|-------------|
| `/` | `index.html` | メインチャットインターフェース（デフォルトキャラクター） |
| `/l2d` | `model_manager.html` | モデル管理（レガシーエイリアス） |
...
| `/cookies_login` | `cookies_login.html` | メディア認証情報管理 |
| `/{lanlan_name}` | `index.html` | キャラクター固有のチャット（キャッチオール） |

::: info
`/{lanlan_name}` キャッチオールルートは同じメインインターフェースを提供しますが、特定のキャラクターを事前に選択します。`/steam_workshop_manager` ページは `?lanlan_name=` クエリパラメータを受け付けます。
:::
